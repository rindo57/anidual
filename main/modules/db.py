...
def save_file_in_db(filed, hash, upid=None, subtitle, img, audio_info):
    filesdb.update_one(
        {
            "hash": hash,
            "fid": str(upid),
        },
        {"$set": {"filename": filed, "filenamex": filed, "code": hash, "msg_id": upid, "subtitle": subtitle, "image": img, "audio": audio_info}},
        upsert=True,
    )
    return
