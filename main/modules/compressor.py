...
async def gg():
          cmd = '''ffmpeg -hide_banner -loglevel quiet -progress "progressaa.txt" -i "video.mkv" -filter_complex "[0:v]drawtext=fontfile=font.ttf:text='t.me/animxt':fontsize=35:fontcolor=ffffff:alpha='if(lt(t,0),0,if(lt(t,5),(t-0)/5,if(lt(t,15),1,if(lt(t,20),(5-(t-15))/5,0))))':x=w-text_w-15:y=15" -c:v libx265 -s 1600x900 -pix_fmt yuv420p10le -preset medium -r 24000/1001 -crf 23.2 -x265-params deblock=1,1:aq-mode=2:aq-strength=0.85:no-info=1 -map 0:v -c:a libopus -b:a 96k -map 0:a -c:s copy -map 0:s? "out.mkv" -y''',
