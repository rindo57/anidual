...
async def gg():
          cmd = '''ffmpeg -hide_banner -loglevel quiet -progress "progressaa.txt" -i "video.mkv" -filter_complex "[0:v]drawtext=fontfile=font.ttf:text='t.me/animxt':fontsize=35:fontcolor=ffffff:typewriter=1:delay=500:x=(w-tw)/2:y=(h-th)/2:enable='between(t,0,20)':disable='gt(t,20)'" -c:v libx265 -s 1280x720 -pix_fmt yuv420p10le -preset medium -r 24000/1001 -crf 23.2 -x265-params deblock=1,1:aq-mode=2:aq-strength=0.85:numa-pools=+:no-info=1 -map 0:v -c:a aac -b:a 128k -map 0:a -c:s copy -map 0:s? "out.mkv" -y''',
